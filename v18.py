...
BLOG_TAGS = {
    "IA Geral": sorted({
        ...
        "IA Para Negócios", "IA para Negócios", "IA e Arquelogia", "IA e Justiça",
    ...
    "Empresas & Modelos": sorted({
        ...
        "Gemini", "Grok", "Grok AI", "ChatGPT", "Copilot", "xAI", "X AI",
        "OpenBot", "BigTechs", "Corporações", "AI Big Techs",
    }, key=str.lower),
    ...
    "Conteúdo & Mídia": sorted({
        ...
        "Slides com IA", "Landing Page com IA", "Instagram com IA",
...
KEYWORDS = [
    ...
    # Empresas, Projetos e Organizações
...
CSS = """
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  #tag-selector-section{
    margin:3rem 0 2rem;border-top:2px solid #e5e7eb;padding-top:2rem;
  }
  #tag-selector-section > h2{
    font-size:1.15rem;margin:0 0 1.2rem;color:#374151;
    font-family:system-ui,sans-serif;border:none;padding:0;
  }
  .tag-group{margin-bottom:1.4rem;}
  .tag-group-label{
    font-size:.75rem;font-weight:800;text-transform:uppercase;letter-spacing:.8px;
    color:#6366f1;font-family:system-ui,sans-serif;margin-bottom:.5rem;
  ...
  .tag-btn{
    ...
    padding:.38rem .9rem;border-radius:20px;font-size:.92rem;font-weight:600;
  ...
  #tag-output{
    flex:1;padding:.6rem .9rem;border:1.5px solid #d1d5db;border-radius:8px;
    font-size:.92rem;font-family:system-ui,sans-serif;color:#1e1e2e;
    background:#f9fafb;resize:none;height:2.7rem;line-height:1.6;
  }
  #btn-copy-tags{
    background:#10b981;color:#fff;border:none;padding:.6rem 1.1rem;
    border-radius:8px;font-weight:700;font-size:.92rem;cursor:pointer;
    font-family:system-ui,sans-serif;transition:background .2s;white-space:nowrap;
  }
  ...
  #btn-clear-tags{
    ...
    padding:.6rem .9rem;border-radius:8px;font-weight:700;font-size:.92rem;
...
function insertImageWrap(url, refEl, replaceCard){
  ...
  /* Re-inicializa o overlay da imagem recém inserida */
  prepareImages();
  updateAllBadges();
}
...
(function(){
  ...
  function renderTagOutput(){
    ...
    // Vírgula + espaço após cada tag (incluindo a última)
  ...
  function setupTagSelector(){
    ...
    document.getElementById('btn-copy-tags').addEventListener('click', function(){
      ...
      navigator.clipboard.writeText(val).then(() => {
        ...
        document.getElementById('tag-output').select();
...
def build_tag_selector_html():
    """Gera o painel HTML de seleção de tags, agrupado por categoria."""
    groups_html = ""
    for group_name, tags in BLOG_TAGS.items():
        pills = ""
        for tag in tags:
            safe = tag.replace('"', '&quot;').replace("'", "&#39;")
            pills += f'<button class="tag-btn" data-tag="{safe}">{safe}</button>\n      '
        groups_html += f"""
  <div class="tag-group">
    <div class="tag-group-label">{group_name}</div>
    <div class="tag-group-pills">
      {pills}
    </div>
  </div>"""
    return f"""
<section id="tag-selector-section">
  <h2>🏷️ Selecionar Tags</h2>
  <div id="tag-buttons-wrap">
    {groups_html}
  </div>
...
def build_html(title, meta, body_html, pixabay_key):
    ...
    tag_selector = build_tag_selector_html()
