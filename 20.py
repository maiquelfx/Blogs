...
BLOG_TAGS = {
    "IA Geral": sorted({
        "IA", "AGI", "Inteligência Artificial", "IA Generativa", "IA interessante",
        "IA Para Negócios", "IA e Arquelogia", "IA e Justiça",
        "IA e Saúde", "IA e Viagens", "IA na Contabilidade", "Inutilidades de IA",
        "LLM", "LL", "Machine Learning", "Deep Learning", "Redes Neurais",
        "Algoritmos", "Alucinação de IA", "Mitos da IA", "Hype da IA",
        "Curiosidades de IA", "Curiosidades da IA", "Erros da IA", "Segredos da IA",
        "Evolução da IA", "História da IA", "História com IA", "Gigantes da IA",
        "Domínios de IA", "Domínios", "Agentes de IA", "Concursos de IA", "Concursos",
        "Benchmarks", "Projetos de IA", "SAI",
    }, key=str.lower),
    "Empresas & Modelos": sorted({
        "OpenAI", "Anthropic", "Google", "Google AI", "Meta AI", "Microsoft AI",
        "Gemini", "Grok", "Grok AI", "ChatGPT", "Copilot", "xAI",
        "OpenBot", "BigTechs", "Corporações", "AI Big Techs","Claude",
    }, key=str.lower),
    "Técnico & Dev": sorted({
        "Python", "Programação", "Programação com IA", "Código com IA",
        "Linguagem de programação", "Linguagem de Programação",
        "GitHub", "Git", "Controle de versão", "Docker", "Frameworks",
        "DEV", "IDE", "CD/CI", "Servidores", "Infraestrutura", "Redes",
        "AI Code", "AI Dev", "AI DOCS", "WEB", "SEO", "Engenharia de Prompt",
        "Prompt", "Integração com IA", "Automação com IA",
    }, key=str.lower),
    "Negócios & Mercado": sorted({
        "Negócios", "Mercado", "Mercado de IA", "Mercado de Trabalho em IA",
        "Marketing", "Marketing com IA", "Marketing Digital", "Finanças",
        "Empreendedorismo", "Renda com IA", "Economia", "Ciências Econômicas",
        "Potfólio", "Linkedin", "Linkedin com IA", "Network",
        "Produtos com IA", "Serviços com IA", "IA Para Negócios",
    }, key=str.lower),
    "Conteúdo & Mídia": sorted({
        "Tutoriais", "Dicas", "Tips", "News", "Notícias", "NEWS", "Resenhas",
        "Vídeos de IA", "Vídeos com IA", "Artigos de IA", "Artigos em inglês",
        "Cursos", "Livros de IA", "Insight", "Insights", "Postagens com IA",
        "Slides com IA", "Landing Page com IA",
        "Youtube com IA", "Sumarização com IA", "Edição com IA",
        "Música com IA", "Músicas com IA", "Arte com IA", "Artes com IA",
        "Imagens", "Design", "Suno",
    }, key=str.lower),
    "Sociedade & Futuro": sorted({
        "Governo", "Governo e IA", "Educação", "Saúde Com IA", "Saúde com IA",
        "Direitos Autorais", "Políticas Públicas para IA", "Religião e IA",
        "Psicologia e IA", "Cultura e IA", "Processos Judiciais de IA",
        "AI Copyright", "Conspirações", "Conspirações com IA",
        "Universidades de IA", "Universidades", "Graduação de IA", "Graduação em IA",
        "Faculdades de IA", "Carreira de IA", "Futuro com IA",
        "Emoções na IA", "Cientistas", "Cientistas da Computação",
        "Biografias", "Bio", "Vida", "Idioma", "Inglês",
    }, key=str.lower),
    "Ferramentas & Apps": sorted({
        "Ferramentas", "Ferramentas de IA", "Apps", "Aplicativos",
        "Dashboards com IA", "Chrome AI", "AI Games", "Playgrounds",
        "Robótica", "Robótica com IA", "Dieta com IA",
        "ai", "Criptografia com IA", "Criptomoedas", "Blockchain",
        "Big Data", "Big O", "Complexidade Algoritmíca", "Complexidade Asssintótica",
        "Matemática", "Ciência com IA","API",
    }, key=str.lower),
    "Outros": sorted({
        "Ads", "Anúncios", "Doação", "Inovação", "Tecnologia", "TI",
        "AI Hackers", "AI News", "AI Security", "AI Domains",
        "Hackers", "Segurança em TI", "ChatBot", "ChatBot com IA", "Chatbox",
        "Entretenimento", "Filmes de IA", "Produzir com IA",
    }, key=str.lower),
}
...
def build_faq_accordion(pairs, platform="blogger"):
    if platform == "wordpress":
        ...
        for i, (pergunta, resposta) in enumerate(pairs, 1):
            ...
            html += (
                f'<p><strong>{i}. {safe_p}</strong><br/>{safe_r}</p>\n'
            )
        html += '</div>'
        return html

    # Blogger — <details> com efeitos visuais via CSS inline
    ...
    for pergunta, resposta in pairs:
        ...
        html += (
            ...
            f'border-top:1px solid currentColor;font-size:1.1rem;">'
...
function applyAutoLinks(portalId){
  ...
  const DOMAIN_LINKS = [
    { pattern: /https?:\/\/ia\.pro\.br\S*/gi,                     href: 'https://ia.pro.br' },
    { pattern: /\bia\.pro\.br(?![\w\/\-])/gi,                     href: 'https://ia.pro.br' },
    { pattern: /https?:\/\/(?:www\.)?maiquelgomes\.com\.br\S*/gi, href: 'https://maiquelgomes.com.br' },
    { pattern: /\bmaiquelgomes\.com\.br(?![\w\/\-])/gi,           href: 'https://maiquelgomes.com.br' },
  ];
    ...
    textNodes2.forEach(textNode => {
      ...
      DOMAIN_LINKS.forEach(({ pattern, href }) => {
        ...
        while((m = pattern.exec(text)) !== null){
          const overlap = domMatches.some(ex => m.index < ex.end && (m.index + m[0].length) > ex.start);
          if(!overlap) domMatches.push({ start: m.index, end: m.index + m[0].length, href, matched: m[0] });
        }
      });
    ...
    nodes.forEach(textNode => {
      ...
      KEYWORDS.forEach(kw => {
        ...
        while((m = regex.exec(text)) !== null){
          const overlap = matchList.some(ex => m.index < ex.end && (m.index + m[0].length) > ex.start);
          if(!overlap){ matchList.push({ start: m.index, end: m.index + m[0].length, kw, matched: m[0] }); usedInBlock.add(kw.toLowerCase()); }
          break;
        }
      });
...
  
  // 1. Remove botões e overlays da ferramenta antes de copiar
...
  });  
  
  // 2. Tratamento da imagem de capa
...
  // 3. O SEGREDO DAS CORES NO BLOGGER ESTÁ AQUI
  // Mapeamento de cores VS Code Dark Theme para o Python
  var TOKEN_COLORS = {
    'keyword': '#569cd6',
    'string': '#ce9178',
    'number': '#b5cea8',
...
  // Força a aplicação do design escuro e das cores antes de jogar pro clipboard
  clone.querySelectorAll('pre').forEach(function(pre){
    pre.style.cssText = 'background-color:#1e1e1e; color:#d4d4d4; padding:15px; border-radius:8px; overflow-x:auto; font-family:Consolas, "Courier New", monospace; font-size:14px; line-height:1.5; margin:1.5rem 0;';
    
    var code = pre.querySelector('code');
    if(code){
      code.style.cssText = 'background:transparent; padding:0; color:inherit; font-family:inherit;';
      
      code.querySelectorAll('span').forEach(function(span){
        var isColored = false;
        Object.keys(TOKEN_COLORS).forEach(function(type){
          if(!isColored && span.classList.contains(type)){
            span.style.color = TOKEN_COLORS[type];
            isColored = true;
          }
        });
        // REMOVER a classe é fundamental para o painel do Blogger não quebrar o HTML!
        span.removeAttribute('class');
      });
    }
  });

  // 4. Envia o conteúdo processado para a área de transferência
...
    btn.innerText = '✅ Copiado com Cores (Cole no Blogger)!'; 
...
(function(){
  ...
  function renderTagOutput(){
    ...
    if(!out) return;
    if(selectedTags.size === 0){ out.value = ''; if(count) count.textContent = 'Nenhuma tag selecionada'; return; }
    out.value = [...selectedTags].join(', ') + ',';
    if(count) count.textContent = `${selectedTags.size} tag(s) selecionada(s)`;
  }
  ...
  function setupTagSelector(){
    ...
    wrap.querySelectorAll('.tag-btn').forEach(btn => {
      btn.addEventListener('click', function(){
        ...
        if(selectedTags.has(tag)){ selectedTags.delete(tag); this.classList.remove('selected'); }
        else { selectedTags.add(tag); this.classList.add('selected'); }
        renderTagOutput();
      });
    });
    ...
    document.getElementById('btn-clear-tags').addEventListener('click', function(){
      selectedTags.clear();
      wrap.querySelectorAll('.tag-btn.selected').forEach(b => b.classList.remove('selected'));
      renderTagOutput();
...
def process_markdown_file(filepath):
    ...
    def replace_faq_tag(m):
        ...
        for pair in pairs_raw:
            ...
            if len(lines) < 2:
                continue
            pergunta = lines[0].rstrip('?') + '?'
            resposta = ' '.join(lines[1:])
    ...
    raw = re.sub(r'\[FAQ\]\s*\n(.*?)\n\[/FAQ\]', replace_faq_tag, raw, flags=re.DOTALL|re.IGNORECASE)
