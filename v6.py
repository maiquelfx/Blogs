...
def upload_github(webp_bytes, filename):
    """
    Faz upload para GitHub e retorna a URL raw.githubusercontent.com.
    Essa URL é permanente, pública e não depende de CDN externo.
    """
    ...
    try:
        ...
        if r.status_code in (200, 201):
            # ✅ Usa raw.githubusercontent.com — sem CDN, sempre funciona
            return (
...
CSS = """
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  /* botão de upload aparece só quando imagem não está no GitHub */
...
# ─────────────────────────────────────────────
#  JS DO EDITOR
#  Recebe GITHUB_TOKEN, GITHUB_OWNER, GITHUB_REPO, GITHUB_FOLDER
#  via placeholders substituídos em build_html()
# ─────────────────────────────────────────────
JS_EDITOR = r"""
<script>
/* ════════════════════════════════════════════════════
   ...
   Novidades:
     • Upload de imagem externa → GitHub direto pelo browser
     • Badge indicando se imagem já está no GitHub ou é externa
...
/* ══════════════════════════════════════════════════
   UPLOAD DE IMAGEM EXTERNA → GITHUB
   Faz fetch da imagem atual, converte para base64
   e usa a API do GitHub para salvar permanentemente.
══════════════════════════════════════════════════ */
async function uploadCurrentToGitHub(btn){
  ...
  try {
    /* 1. Busca a imagem via proxy CORS ou diretamente */
    let imageData;
    try {
      ...
      /* Fallback: canvas (só funciona se CORS permitir) */
    ...
    if(resp.status === 201 || resp.status === 200){
      /* 5. Atualiza o src da imagem para a URL raw permanente */
